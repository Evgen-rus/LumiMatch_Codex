...
Основной отчёт содержит только подтверждённые `in_stock` позиции. В `sample-run` это обеспечивают pool и live recheck с `availability_policy="sellable"`; `write_reports` сам не фильтрует переданные результаты. Внутренняя supplier capability для `stock_not_published` не отменяет этот контракт. Независимый `*_unavailable` отчёт предназначен для диагностики visual matching и не является предложением клиенту.
...
По последнему сохранённому golden benchmark основная измеренная потеря находится между URL inventory и hydrated catalog. Ранний CPU visual spike не подтвердил замену family-aware retrieval и Codex review. Более поздний [controlled benchmark](docs/CONTROLLED_VISUAL_BENCHMARK.md) показал полезность embeddings на отдельных референсах, но не доказал автоматическую замену проверки конструкции.
...
## 9. Детали работы с проектом

`FixtureRequirement`, `SourceRef`, `CatalogProduct`, `ScoredCandidate` описаны в `lumimatch/models.py`: это единственный полный список полей, типов и defaults. Требование сохраняет помещение, тип/количество, визуальное описание, цвет/форму/монтаж, известные размеры, технические ограничения, ссылки на источники и crops, confidence. Неизвестное оставляется неизвестным.

Классы результата: «вероятное точное совпадение», «очень близкий аналог», «похожий вариант». Цель — 3–5 достойных вариантов при наличии, но допустимы 0–5; слабые варианты не добавляются ради количества. Приоритет UX — понятный HTML/Markdown-отчёт и локальные данные, без тяжёлого frontend; лёгкий UI допустим лишь при реальном сокращении работы.

Кэшировать каталог, изображения, обработанные PDF, требования, shortlist и предыдущие результаты; live recheck финалистов остаётся без кэша. Операционные команды и проверки — [DEVELOPMENT](docs/DEVELOPMENT.md), выбор моделей — только [AGENTS](AGENTS.md).
