...
В `data/visual_review.json` хранится ручная визуальная проверка Codex. Для sample V2 основной pool и contact sheets находятся в `output/review/<requirement>/`, диагностический availability-independent pool — в `output/review_unavailable/<requirement>/`. Основной отчёт содержит только подтверждённые `in_stock`; отдельные `output/lumimatch_sample_unavailable.{json,md,html}` показывают только достойные визуальные аналоги со статусами `out_of_stock`, `preorder`, `expected` и `check_availability`. Явный reject никогда не остаётся ни в одном финальном отчёте. Если хороших вариантов нет, отчёт намеренно показывает `Ничего не найдено. Попробуйте подобрать вручную.`.
...
Перед финалом система повторно открывает публичную карточку без кэша. Основной отчёт требует актуальную `in_stock`; второй отчёт проверяет URL, SKU, актуальность карточки и отсутствие discontinued/archive, но принимает только четыре явно разрешённых временных статуса. `stock_not_published` остаётся supplier-level capability для консервативной политики каталога и не ослабляет основной пользовательский отчёт.
